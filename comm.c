...
// The function chould use these values  to re-calculate 8-pieces of fst decibel values.
...
void sendToServer(double rms[]){
	...
	for(i=0; i<8; i++){		// outer loop repeas 8 times for 8-pieces of result
		for(j=0; j<10; j++){		// inner loop repeats 10 times to accumulate
			sum = sum + (rms[j+i*10]*rms[j+i*10])*200;
		}
